...
Раздел **«Расписание»** предназначен для планирования занятости залов во времени: занятия с студентами и преподавателями, аренда, техническое обслуживание, блокировки слотов. От сюда же для занятий типа «Занятие» ведётся **фактическая посещаемость** по конкретным датам вхождения серии.
...
Роли **OWNER**, **ADMIN** и **INSTRUCTOR** могут создавать и редактировать записи (с оговоркой по удалению ниже). **STUDENT** и **SUB_TENANT** обычно только просматривают календарь; попытка сохранить новую запись под студентом завершится отказом сервера.
...
- **Тип** — обязательный выбор из списка:
  ...
  - **Занятие** — урок; требуется предмет и связка с преподавателями/студентами на других вкладках.
...
Вторая вкладка — **«Студенты»**.

- Выпадающий список **«Выберите студента»** с поиском.
- Кнопка **«Добавить»** (с плюсом) добавляет выбранного студента в таблицу ниже.
- Таблица выбранных студентов; если пусто — текст **«Студенты не добавлены»**.

Студенты берутся из членов организации с ролью студента.
...
На вкладке таблица участников на **конкретное вхождение** (конкретные `startTime` / `endTime` экземпляра в календаре): преподаватели и студенты с отметками присутствия. Сохранение выполняется элементами редактора посещаемости (кнопки внутри вкладки согласно текущей версии UI).
...
5. Время с/по.
6. Вкладка **«Студенты»** — добавить студентов.
...
5. Время с/по, студенты, преподаватели.
6. **«Создать»**.
...
- **«Пропуска»** и станция QR используют расписание для проверки «есть ли слот в допустимом окне» при входе студента/преподавателя.
