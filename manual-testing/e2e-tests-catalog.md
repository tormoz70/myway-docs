...
| E2E-005 | TC-GEN-005 | публичная заявка студента с тарифом | — | На `/myway/{slug}/join/student` выбирает предмет и тариф из seed, заполняет форму, отправляет. Проверяет успешное завершение заявки. Нужен `E2E_SEED_SUBJECT_NAME` с утверждённым тарифом. |
...
| E2E-018 | TC-OWN-004 | приглашение администратора | OWNER | **«Сотрудники»** → «Администраторы» → «Пригласить» → email `admin-invite-{ts}@example.com` → toast «…данные для входа отправлены на почту», строка в таблице. Полный цикл с письмом и первым входом — TC-OWN-004A (ручной / SMTP). |
| E2E-019 | TC-OWN-005 | преподаватели: приглашение (smoke) | OWNER | **«Сотрудники»** → «Преподаватели» → «Пригласить» → email `instr-invite-{ts}@example.com` → toast приглашения (без проверки почты). |
...
| E2E-025 | TC-OWN-011 | пропуска: шаблон и выдача | OWNER | Раздел «Пропуска»: создаёт шаблон пропуска и выдаёт пропуск студенту (helper `createPassTemplateAndIssue`). |
...
### 1.9. Студент — `manual/student.spec.ts`
...
| E2E-055 | TC-PRV-002 | удаление аккаунта (destructive) | STUDENT | API: публичная заявка студента → одобрение OWNER → вход → «Удалить аккаунт» с подтверждением email. Создаёт и удаляет одноразового пользователя. |
